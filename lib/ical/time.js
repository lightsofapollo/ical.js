...
(function() {
  ...
  ICAL.Time.prototype = {
    ...
    dayOfWeek: function icaltime_dayOfWeek(startingDay) {
      ...
      // the math below will work just fine if the starting time of Sunday (1) is passed.
      if (startingDay) {
        // subtract the difference
        h -= (startingDay - ICAL.Time.SUNDAY);

        // if the value is less then 0 then restart from one with the difference.
        if (h <= 0) {
          h = 7 + h;
        }
      }
    ...
    adjust: function icaltime_adjust(aExtraDays, aExtraHours,
      ...
      time.day += aExtraDays || 0;
      time.hour += aExtraHours || 0;
      time.minute += aExtraMinutes || 0;
      time.second += aExtraSeconds || 0;

      if (
        time.month > 0 && time.month < 13 &&
        time.day > 0 && time.day < 29 &&
        time.hour > -1 && time.hour < 25 &&
        time.minute > -1 && time.minute < 60 &&
        time.second > -1 && time.second < 60
      ) {
        // abort the time does not require adjustment...
        return;
      }
